...
\State $r \gets 0$ 
\State $f \gets p$
\State $q_s \gets$ empty collection of length $n$ 

\While{$f \neq 0$}
    \State $i \gets 0$
    \State divionOccured $\gets $ false

    \While{$i \leq n$ AND divionOccured $=$ false}
        \If{$\operatorname{LM}(f) \bmod \operatorname{LM}(g_i) = 0$}
            \State $t \gets f/g_i$
            \State $q_i \gets q_i + t$
            \State $f \gets f - tg_i$
            \State divionOccured $=$ true 
        \Else
            \State $i \gets i + 1$
        \EndIf
    \EndWhile
